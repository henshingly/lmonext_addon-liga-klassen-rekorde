...
## Version 1.7.1 (KRITISCHER Bugfix)

- KRITISCHER Bugfix (gemeldet: "Sicherheitsprüfung fehlgeschlagen: die Datei 'lmo-rekorde.php' enthält keinen gültigen PHP-Code. Installation abgelehnt."): beim Einfügen der neuen Helper-Funktionen (rkBuildTeamNameDirectory()/rkPickName()/rkTeamLogoImg()/rkTeamCell()) in Version 1.7.0 wurde versehentlich die öffnende Docblock-Zeile "/**" des direkt folgenden rkMeisterliste()-Kommentars mit entfernt - der Rest des Kommentartexts stand dadurch außerhalb jedes gültigen PHP-Konstrukts, was zu einem echten Parse-Fehler führte (von PHP selbst als "unexpected token '*'" erkannt, nicht nur ein Stilproblem). Ein reiner Klammern-Zähl-Check hatte das nicht erkannt, da die Gesamtzahl der Klammern unverändert blieb - ab sofort wird zusätzlich der tatsächliche PHP-Parser (php -l) zur Verifikation herangezogen. Alle Dateien dieses Pakets damit erneut geprüft: keine Syntaxfehler.

## Version 1.7.0 (Neues Feature)

- Neues Feature (auf Wunsch): zwei neue, PRO KLASSE einstellbare Anzeige-Optionen für Meisterliste und Rekordspiele - "Logos anzeigen" (an/aus) und "Teamname: kurz/mittel/lang". Einstellbar über Administrator → Liga-Klassen → Klasse bearbeiten.
- Datenbank-Migration (handler_ligaklassen.php 1.4.0): zwei neue, additive Spalten auf liga_klassen (show_logos TINYINT(1) DEFAULT 0, team_name_mode ENUM('kurz','mittel','lang') DEFAULT 'kurz') - eigener, separater Migrationsblock mit eigenem Flag (liga_klassen_schema_v2), tastet die bestehende v1-Migration nicht an. Bestehende Klassen starten mit Logos aus und Kurzform, unverändertes bisheriges Verhalten.
- lmo-rekorde.php 1.14.0: neues, zentrales Team-Namensverzeichnis (rkBuildTeamNameDirectory()) je Klasse statt einer Erweiterung jeder einzelnen Datenquellen-Funktion einzeln - eine Helper-Funktion (rkTeamCell()) kombiniert Logo (rkTeamLogoImg(), gleiches Muster wie im ewige-tabelle-Addon) und gewählte Namenslänge (rkPickName(), mit Rückfall auf die Langform, falls Kurz-/Mittelform bei einem Team nicht gepflegt ist) konsistent für ALLE Ansichten: Meisterliste (inkl. Meister-Statistik-Tabelle), alle Rekordspiele-Listen (höchster Sieg, ergebnisreichste Partie, höchstes Unentschieden), beide Team-Zähl-Tabellen (ohne Gegentore/ohne eigene Tore) und alle acht Serienarten (inkl. der Tabellenführungs-Serie).
- Neue Sprachschlüssel: lk_label_show_logos, lk_label_team_name_mode, lk_option_name_kurz/mittel/lang.
